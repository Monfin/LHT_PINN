...
from src.data.components.collate import ModelInput, ModelBatch, ForwardState, Coords
from src.models.components.utils.conditions import PDEXYConditions, InitialConditions, BoundaryXYZConditions
...
class PDELitModule(L.LightningModule):
    def __init__(
            ...
            pdec: PDEXYConditions,
            ic: InitialConditions = None, 
            bc: BoundaryXYZConditions = None,
            num_coords: int = 1,
    ...
    def forward(self, inputs: ModelInput) -> ForwardState:
        return self.net(inputs)
    

    def pde(self, u_xx, u_yy):
        # Poisson's equation
        return self.alpha * u_xx[-1] + self.beta * u_yy[-1]
    ...
    def pde_forward(self, inputs: ModelInput):
        u = self.forward(inputs)

        if u.logits.requires_grad:

            u_xx = self.derivative(u.logits, inputs.coords.x, 2)
            u_yy = self.derivative(u.logits, inputs.coords.y, 2)

            pde = self.pde(u_xx, u_yy)

            loss = self.criterion(pde, self.hparams.pdec(inputs))
    ...
    def ic_forward(self, inputs: ModelInput): # IC loss
        ...
        ic_u = self.forward(inputs)

        return ic_u, self.ic_criterion(ic_u, self.hparams.ic(inputs.coords))
    
    
    def bc_forward(self, inputs: ModelInput): # BC loss
        _coords = inputs.coords.__dict__.items()
        bc_u, loss_bc = list(), list()

        for (key, coord), criterion in zip(_coords, self.bc_criterion):
            if coord is not None:
                bc_inputs = ModelInput(
                    coords=Coords(
                        **{
                            nested_key: nested_coord if nested_key != key else torch.zeros(size=nested_coord.size()) \
                ...
                loss_bc.append(criterion(bc_u[-1].logits, self.hparams.bc(coord)))
    ...
    def model_step(self, batch: ModelBatch) -> Tuple[torch.Tensor]:
        ...
        u, pde, loss_pde = self.pde_forward(inputs)

        loss = loss_pde
        branched_loss = [loss_pde.detach()]

        if self.ic_criterion is not None:
            _, loss_ic = self.ic_forward(inputs)

            loss += loss_ic
            branched_loss.append(loss_ic.detach())

        if self.bc_criterion is not None:
            _, loss_bc = self.bc_forward(inputs)

            loss += sum(loss_bc)
            branched_loss.extend([loss_item.detach() for loss_item in loss_bc])
